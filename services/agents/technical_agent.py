...
class TechnicalAgent(IAgent):
    name='Technical'; default_weight=0.18
    def propose(self, ticker:str, sentiment:Optional[float]=None, price:Optional[float]=None)->AgentProposalResp:
        ...
        return AgentProposalResp(agent=self.name, vote=vote, weight=self.default_weight, confidence=conf, rationale='Pris/volymsignal (stub).', features=[f'RSI {int(rsi)}', 'MA20>MA50' if above else 'MA20<=MA50'])
