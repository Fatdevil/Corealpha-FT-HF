...
class MacroAgent(IAgent):
    name='Macro'; default_weight=0.10
    def propose(self, ticker:str, sentiment:Optional[float]=None, price:Optional[float]=None)->AgentProposalResp:
        ...
        return AgentProposalResp(agent=self.name, vote=vote, weight=self.default_weight, confidence=conf, rationale='Makroregim (stub).', features=['Risk-on' if risk_on else 'Risk-off'])
