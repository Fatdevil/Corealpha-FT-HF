...
class FundamentalAgent(IAgent):
    name='Fundamental'; default_weight=0.20
    def propose(self, ticker:str, sentiment:Optional[float]=None, price:Optional[float]=None)->AgentProposalResp:
        ...
        return AgentProposalResp(agent=self.name, vote=vote, weight=self.default_weight, confidence=conf, rationale='Värdering vs kvalitet (stub).', features=[f'PE fwd {pe:.1f}x', f'GM {int(gm*100)}%'])
