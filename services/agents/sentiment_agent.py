...
class SentimentAgent(IAgent):
    name='Sentiment'; default_weight=0.22
    def propose(self, ticker:str, sentiment:Optional[float]=None, price:Optional[float]=None)->AgentProposalResp:
        ...
        return AgentProposalResp(agent=self.name, vote=vote, weight=self.default_weight, confidence=conf, rationale='Nyhetston (stub).', features=[f'Ton {s:+.2f}'])
